...
static int rtmp_write(URLContext *s, const uint8_t *buf, int size)
{
    ...
    do {
        if (rt->skip_bytes) {
            // doublesky 第一次进来buf是指向的flv数据流头类似于flv文件头 因此skip_bytes为13跳过FLV Header(9字节) + Previous Size(4字节) 意思是跳到了一个tag 一般都是flv信息tag tagtype为0x12
        ...
        if (rt->flv_header_bytes < RTMP_HEADER) {
            ...
            // doublesky 将buf_temp的数据复制到rt->flv_header里去 并且buf_temp往后移 rt->flv_header就是当前tag的tag header
            ...
            // doublesky rt->flv_size就是tag header的data size大小
            rt->flv_size = pktsize;
            ...
            if (((pkttype == RTMP_PT_VIDEO || pkttype == RTMP_PT_AUDIO) && ts == 0) ||
                ...
                if (pkttype == RTMP_PT_NOTIFY) // doublesky rtmp协议中的setDataFrame指令吧 字符串13字节 下面ff_amf_write_string有写入
                    pktsize += 16;
            ...
            // doublesky rt->flv_data与rt->out_pkt.data数据可以同步
        ...
        if (rt->flv_size - rt->flv_off > size_temp) {
            // doublesky buf_temp复制到rt->flv_data里去
        ...
        // doublesky 输出完之后跳过previous tag size的大小 因此为4字节
        if (rt->flv_off == rt->flv_size) {
            rt->skip_bytes = 4;
