...
static int flv_write_header(AVFormatContext *s)
{
    ...
    // doublesky这里可以看到duration_offset暂时只保存了位置 没有写入数据
    ...
    // doublesky 这里可以看到FLVContext的filesize_offset指向了filesize后面 暂时没写入数据
    ...
    // doublesky metadata_size_pos是onMetaData数据前的第一个FLV Tag头的datasize位置
    /* write total size of tag */
    data_size = avio_tell(pb) - metadata_size_pos - 10;

    // doublesky 更新metadata_count metadata_count_pos在onMetaData后的AMFType的后面 也就是onMetaData+1个字节后
    avio_seek(pb, metadata_count_pos, SEEK_SET);
    avio_wb32(pb, metadata_count);

    // doublesky 更新metadata的tag header中的size字段
    avio_seek(pb, metadata_size_pos, SEEK_SET);
    avio_wb24(pb, data_size);
    avio_skip(pb, data_size + 10 - 3);
    
    // doublesky 上一个tag的大小 即flv的previous tag size
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (enc->codec_id == AV_CODEC_ID_AAC || enc->codec_id == AV_CODEC_ID_H264 || enc->codec_id == AV_CODEC_ID_MPEG4) {
            ...
            if (enc->codec_id == AV_CODEC_ID_AAC) {
                ...
                // doublesky 视频tag data写入 这里代表着调用这个函数前 必须已经传入了sps跟pps信息
                avio_w8(pb, enc->codec_tag | FLV_FRAME_KEY); // flags
                avio_w8(pb, 0); // AVC sequence header
                avio_wb24(pb, 0); // composition time
                ff_isom_write_avcc(pb, enc->extradata, enc->extradata_size);
            }
            // doublesky pos是在tag header后面
...
{
    // doublesky 这里刚开始pb的buffer里就有了flv头和flv文件第一个tag的信息了
    AVIOContext *pb      = s->pb;
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        // doublesky 这里可以看到写入的AVPacket是否是关键帧一定要传入 这个flag参数是视频Tag Data的第一个字段 高四位表示是否关键帧 低四位代表视频编码格式
    ...
    if (enc->codec_type == AVMEDIA_TYPE_DATA) {
        ...
        av_assert1(flags>=0);
        // doublesky 音视频的flv tag data第一个字节开始写入
        avio_w8(pb,flags);
